...
module Koans
  ...
  def self.remove_solution(line)
    line = line.gsub(%r{\b____\([^\)]+\)}, '____')
    line = line.gsub(%r{\b___\([^\)]+\)}, '___')
    line = line.gsub(%r{\b__\([^\)]+\)}, '__')
    line = line.gsub(%r{\b_n_\([^\)]+\)}, '_n_')
    line = line.gsub(%r(/\#\{__\}/), '/__/')
    line = line.gsub(%r{\s*#\s*__\s*$}, '')
    line
  end
...
module RubyImpls
  ...
  def self.find_ruby_impls
    rubys = `rvm list`.gsub(%r{=>}, '').split(%r{\n}).map(&:strip).reject { |x| x.empty? || x =~ %r{^rvm} }.sort
